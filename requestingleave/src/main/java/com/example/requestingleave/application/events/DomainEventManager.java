...
public class DomainEventManager {
    ...
    private void sendRemoteEvent(Event event) {
        String exchange = env.getProperty("rabbitmq.leave_exchange");

        if (event instanceof LeaveRequestStartedEvent) {
            String routingKey = env.getProperty("rabbitmq.leaveRequestStartedKey");
            sender.convertAndSend(exchange, routingKey, event);
        } else if (event instanceof LeaveRequestCancelledEvent) {
            String routingKey = env.getProperty("rabbitmq.leaveRequestCancelledKey");
            sender.convertAndSend(exchange, routingKey, event);
        } else {
